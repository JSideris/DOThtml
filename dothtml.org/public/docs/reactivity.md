...
Side effects allow you to perform non-UI tasks (like logging, API calls, or manual DOM manipulation) in response to state changes. In DOThtml, side effects are managed using `dot.effect()`.

### Basic Usage

An effect runs immediately when created, and then re-runs whenever any of the signals it accesses change.
...
count.value++; // Logs: "The count is now: 1"
```

### Automatic Dependency Tracking

Just like computed signals, effects automatically track their dependencies. You don't need to provide a dependency array. If your effect logic branches, DOThtml will only subscribe to the signals that were actually accessed during the last execution.

### Cleanup Pattern

If your side effect sets up a resource that needs to be cleaned up (like a timer or an event listener), you can return a cleanup function from the effect callback. This function will run before the effect re-executes and when the effect is disposed.
...
dot.effect(() => {
    ...
    // Cleanup: stop the timer when isActive becomes false or effect is disposed.
...
When `dot.effect()` is called inside a component's `constructor()` or `build()` method, or inside a Store's initialization, it is automatically tied to that owner's lifecycle. When the component is unmounted or the store is disposed, the effect is automatically stopped and its cleanup function is called.

If you create an effect outside of these contexts, `dot.effect()` returns a manual dispose function:

```javascript
const dispose = dot.effect(() => { /* ... */ });
dispose(); // Manually stop the effect.
```

### Batched Updates

Effects are integrated with DOThtml's update scheduler. If multiple signals change in the same execution tick, the effect will only re-run once at the end of the tick, ensuring optimal performance.
