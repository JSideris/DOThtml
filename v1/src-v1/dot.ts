...
const dot: IDotCore = (function(targetSelector: string|Element|Node|NodeList|Array<Node|Element>, targetWindow = window)
{

	if((targetSelector as any)?.ownerDocument?.defaultView){
		// It's an element.
	}

	//console.log(targetSelector);
	var targets = targetSelector ? (
		typeof targetSelector == "string" ? targetWindow.document.querySelectorAll(targetSelector) 
		: (targetSelector instanceof targetWindow.Element || targetSelector instanceof targetWindow.Node ? [targetSelector] 
			: ((targetSelector instanceof targetWindow.NodeList || (targetSelector instanceof targetWindow.Array)  && targetSelector[0] && (targetSelector[0] instanceof targetWindow.Element || targetSelector[0] instanceof targetWindow.Node)) ? targetSelector as []
				: []
	)))
	: [];

	//console.log(targets);
	var newDot = new DotDocument(undefined, undefined, targetWindow);

	// We could technically support multiple targets, but this introduces a number of hazards. A single target is how dot was intended to be used.
	if(targets.length > 0){
...
function deleteElement(element: Element){
	var deleted = null;
	var dc: IWrappedComponent = element["__dothtml_component"];
	if(dc){
		// var d = dc.deleting;
...
// 	var startTextNode = T.__window.document.createTextNode("");
// 	var endTextNode = T.__window.document.createTextNode("");
...
// 	let lastComponent: IWrappedComponent = dot["__lastComponent"];
...
// TODO: the best thing to do would be to attach the curent document and window to the dot document.
// const documentStack: Array<Document> = [document];
// const windowStack: Array<any> = [window];
// dot.setTargetWindow = function(target: Window){
// 	windowStack.push(target);
// 	documentStack.push(target.document);
// 	return(dot(target.document))
// }
// dot.unsetTargetWindow = function(){
// 	documentStack.pop();
// 	windowStack.pop();
// 	if(documentStack.length == 0){
// 		throw new Error("Too many calls to dot.unsetTargetWindow.");
// 	}
// }
